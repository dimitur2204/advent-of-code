...
func extractNumber(row string, index int) (int, int, int) {
	row = strings.TrimSpace(row)
	number := ""
	current := rune(row[index])
    startIndex := -1
    endIndex := -1
	for true {
		if index == 0 || !unicode.IsDigit(rune(row[index])){
			break
		}
		index--
		current = rune(row[index])
	...
	// if a . or symbol is hit move to the right
    ...
    startIndex = index
	for true {
		if index == len(row) || !unicode.IsDigit(rune(row[index])) {
			break
		}
		current = rune(row[index])
	...
	return num, startIndex, endIndex
}
