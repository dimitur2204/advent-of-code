...
func getMinimumSet(game Game) Set {
	...
	for _, set := range game.sets {
		if minimumSet.red < set.red {
			minimumSet.red = set.red
		}
		if minimumSet.blue < set.blue {
			minimumSet.blue = set.blue
		}
		if minimumSet.green < set.green {
...
func parseInput(input string) (games []Game) {
	fullGameStrings := strings.Split(input, "\n")
	for _, fullGameString := range fullGameStrings {
		if len(fullGameString) == 0 {
			continue
		}
		game := Game{}
		gameId := strings.Split(strings.Split(fullGameString, ":")[0], " ")[1]
		game.id, _ = strconv.Atoi(gameId)
		gameString := strings.Split(fullGameString, ":")[1]
		sets := strings.Split(gameString, ";")
		for _, setString := range sets {
			set := Set{}
			colors := strings.Split(setString, ",")
			for _, colorNumberStr := range colors {
				colorNumberStr = strings.TrimSpace(colorNumberStr)
				color := strings.Split(colorNumberStr, " ")[1]
				numberStr := strings.Split(colorNumberStr, " ")[0]
